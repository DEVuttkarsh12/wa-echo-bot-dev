# WhatsApp Cloud API Echo Bot

A production-ready Node.js and Express echo bot for WhatsApp Cloud API. This application handles the webhook verification handshake from Meta, safely validates incoming webhook text payloads, and echoes back messages to the sender.
...
* `src/webhook.js` - Router containing webhook verification handshake (GET) and incoming message processing/validation logic (POST).
...
* `package.json` - Manages project metadata, Node.js engine specification, scripts, and production dependencies.
